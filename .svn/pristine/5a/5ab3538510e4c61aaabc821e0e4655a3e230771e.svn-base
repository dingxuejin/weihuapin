<template>
  <div class="alarmEventBadSteer">
    ...
    <div class="alarmEventBadSteer_content" v-if="period=='y'&&BadSteering&&BadSteering!==null">
      <barhor chartTheme="dark"
              v-if="getBadSteerYearData"
              :data="getBadSteerYearData"
              :color="barhorColor"></barhor>
    </div>
    <div class="alarmEventBadSteer_content" v-if="period=='m'&&BadSteering&&BadSteering!==null">
      <barhor chartTheme="dark"
...
export default {
  ...
  computed: {
    ...
    getBadSteerYearData() {
      return [
        {
          data: this.BadSteering.thisYear.map(item => ({
    ...
    getBadSteerMonthData() {
      return [
        {
          data: this.BadSteering.thisMonth.map(item => ({
            name: item.jssjlx.drive_event_type,
            value: item.total_danger_drive,
          })).sort((a, b) => Number(b.value) - Number(a.value)),
        },
