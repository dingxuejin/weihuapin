...
export default {
  ...
  computed: {
    newOption() {
      ...
      option.yAxis.data = this.data[0].data[0].data.map(item => item.name).reverse()
      // option.yAxis.data = this.data[0].data.map(item => item.name).reverse()
      option.series = this.data[0].data.map((item, index) => ({
        name: item.name,
      ...
      // option.legend.data = this.data[0].data.map(item => item.name)
      // console.log('optio xn.series', option.series)
  ...
  mounted() {
    // console.log('this.data', this.chartOption.series)
  },
}
</script>
