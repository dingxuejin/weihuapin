<template>
  <div class="company_secure_driver_irregularity_distribute_trend_interval_wrap"
       v-if="irregularityDistributeTrendInterval_show">
    <!-- <popup-out-frame title="违规行为时段分布变化趋势"> -->
    <!-- 选择类型 -->
    <div class="company_secure_driver_danger_outframe_wrap"
      ...
      <pop-up-frame @close='closeDetails'>
        <pop-up-widget title="违规行为时段分布变化趋势"></pop-up-widget>
        ...
        <!-- 内容：chart -->
        <div class="irregularity_distribute_trend_interval_chart">
...
export default {
  ...
  computed: {
    ...mapState("popupSecContainer", [
      ...
      "driver_irregularity_distribute_trend_interval"
    ]),
    // 获取chart数据
    getChartData() {
      if (this.driver_irregularity_distribute_trend_interval) {
        return this.driver_irregularity_distribute_trend_interval;
      }
      return null;
    },
    // 设置参数
  ...
  methods: {
    ...mapActions("popupSecContainer", [
      "getdriver_irregularity_distribute_trend_interval"
    ]),
    ...mapMutations("popupSecContainer", [
      "setirregularityDistributeTrendInterval_show"
    ]),
    closeDetails() {
      this.setirregularityDistributeTrendInterval_show(false);
    },
    selectYear(item) {
      this.yearValue = item;
      this.getdriver_irregularity_distribute_trend_interval(
        this.getQueryParams
      );
    },
    selectMonth(item) {
  ...
  mounted() {
    this.getdriver_irregularity_distribute_trend_interval(this.getQueryParams);
  }
};
</script>
...
.company_secure_driver_irregularity_distribute_trend_interval_wrap {
  width: 750px;
  position: absolute;
  top: 325px;
  left: 790px;
  ...
  .irregularity_distribute_trend_interval_chart {
    width: 100%;
    height: 200px;
    margin-top: 10px;
  }
