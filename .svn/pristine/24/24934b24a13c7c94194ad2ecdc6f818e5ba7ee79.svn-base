...
const baseURL1 = process.env.NODE_ENV === 'production' ? 'http://39.104.89.57:8081' : 'http://39.104.89.57:8081'
...
export async function getWorkOrderDetailsInfo({ driver_id, year, month, day }) {
  return axios.get(`${baseURL}/driver_waybill_detail_info`, {
    params: {
      driver_id,
      year,
      month,
...
// 驾驶行为详细信息
export async function getDriverBehaviorAlarmDetailsInfo({ driver_id }) {
...
export async function getIrregalarityBehaviorDetailsInfo({ driver_id }) {
...
export async function getIllegalBehaviorDetailsInfo({ driver_id }) {
...
export async function getBlameAccidentDetailsInfo({ driver_id }) {
