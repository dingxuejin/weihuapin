<template>
  <div>
    <pop-up-widget title="驾驶区间信息"></pop-up-widget>
    <div class="trainDetailsInfoCon">
      ...
      <div class="rewardListCon eleScrollBar">
        <list-component :listData="result"
                        v-if="workMileageDetailsInfo"
...
export default {
  ...
  computed: {
    ...mapState('workmileagedetailsinfo', ['workMileageDetailsInfo']),
    result() {
      return this.workMileageDetailsInfo.map(i => ({
        ...i,
        total_km: i.stop_km - i.start_km,
      }))
    },
  ...
  methods: {
    ...mapActions('workmileagedetailsinfo', ['getWorkMileageDetailsInfo']),
    select(name, item) {
      this[name] = item.value
      this.getWorkMileageDetailsInfo(this.fetchData)
    },
  },
  data() {
    return {
      ...
      titlesData: [
        ...
        { name: '运单编号', key: 'waybill_id', width: 210 },
        { name: '出发地', key: 'start_location', width: 400 },
        { name: '目的地', key: 'stop_location', width: 400 },
        { name: '里程', key: 'total_km', width: 120 },
      ],
    }
  },
}
