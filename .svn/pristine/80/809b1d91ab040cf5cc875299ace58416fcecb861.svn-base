...
const actions = {
  ...
  async fetchOtherBlock({ commit, dispatch }, { entity_id }) {
    ...
    dispatch(
      ...
      { entity_id: state.selectedEntity, month:null, year:null,drive_event_type_code: null },
      { root: true }
    );
    ...
    // 不同周期违规行为数量
    dispatch(
      "popupSecContainer/getDriverIrregualarityCount",
      { entity_id: state.selectedEntity, year: null, month: null },
      { root: true }
    );
    // 不同周期违法行为数量
    dispatch(
      "popupSecContainer/getDriverIllegalCount",
      { entity_id: state.selectedEntity, year: null, month: null },
      { root: true }
    );
    // 事故区域分布
    ...
    // 事故总览
    dispatch("accidentView/getAccidentList",state.selectedEntity, 
    { root: true });
// dispatch(
    //不同类型事故时间分布
    dispatch(
