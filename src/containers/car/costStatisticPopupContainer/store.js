...
const state = {
  ...
  // costInformationStatisticsData: null
};
const mutations = {
  ...
  // setcostInformationStatisticsData(state, data) {
  //   state.costInformationStatisticsData = data;
  // }
};

const actions = {
  async getcarCostStatistics({ commit }, vehicleId) {
    const carCostStatistics = await CarAPI.getcarCostStatistics(vehicleId);
    commit("setCarCostStatistics", carCostStatistics.data);
  },
  async getcarFuelvsMileage({ commit }, vehicleId) {
